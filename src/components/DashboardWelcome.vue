...
const data = [
    { id: 1, value: `${useBaseStore.device?.data?.alias} has been assigned to this device for a period of 30 minutes.` },
...
<template>
    <main
        class="w-10/12 mx-auto h-full bg-white flex flex-col gap-y-16 items-center justify-center md:w-8/12 lg:w-10/12 2xl:w-8/12">

        <AppLogo class="text-5xl text-black" />

        <div class="grid grid-cols-1 gap-6 text-sm sm:grid-cols-2 md:grid-cols-1 lg:grid-cols-2 xl:grid-cols-3">
            <span v-for="hint in data" :key="hint.id"
                class="flex text-center justify-center bg-zinc-100 rounded-sm px-4 py-6">
                <p class="text-zinc-700 text-sm">{{ hint.value }}</p>
            </span>

            <!-- start new chat button -->
            <RouterLink :to="{name: 'newlinq'}" v-if="useChatP2PStore.chatConnect.connected"
